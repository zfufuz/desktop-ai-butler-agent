...
- 导入资料库，基于本地资料问答
- 生成报告、计划、待办清单
- 在时间规划中心使用日视图和周视图安排计划，检测时间冲突并推荐空闲时段
- 将日程完成状态同步回关联计划，并通过专注计时和系统通知推进执行
- 保存个人偏好和长期目标
...
- ChronoDeck 时间规划中心：日/周日历、计划转日程、冲突检测、智能重排、专注计时和完成联动
- 日程 Agent Tool：查询指定日期日程、寻找空闲时段、经权限确认后创建日程
...
React Renderer（日常工作流 / 高级设置）
  -> ChronoDeck 时间规划中心（日/周视图 / 冲突检测 / 专注计时）
  -> Preload 白名单 API
  -> Electron IPC
...
ChronoDeck 没有作为第二套应用运行，而是被拆成桌面管家的时间调度能力。计划仍由工作流模块管理，具体执行时间保存在 SQLite `schedule_events` 表中，两者通过 `planId` 关联。
...
用户既可以从计划卡片点击“安排日程”，也可以直接对 Agent 说“查看明天的安排”“帮我找今天一小时空闲时间”或“把写周报安排到明天 14:00”。查询是只读操作，创建日程沿用现有权限确认和审计日志体系。
